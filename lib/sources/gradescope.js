...
function parseCourses(html) {
  ...
  const boxes = [...html.matchAll(
    /<a[^>]+href="\/courses\/(\d+)"[^>]*class="[^"]*courseBox[^"]*"[^>]*>([\s\S]*?)<\/a>/g
  )].map((m) => {
    const inner = m[2];
    const short = inner.match(/courseBox--shortname[^>]*>([\s\S]*?)<\/(?:h\d|div|span)>/);
    const name = inner.match(/courseBox--name[^>]*>([\s\S]*?)<\/(?:h\d|div|span)>/);
    return {
      id: m[1],
      code: short ? strip(short[1]) : null,
      name: name ? strip(name[1]) : (short ? strip(short[1]) : `Course ${m[1]}`),
      index: m.index,
    };
  });
...
module.exports = { id: 'gradescope', label: 'Gradescope', fetchAll, PARTITION };
