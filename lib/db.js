...
for (const [table, column, ddl] of [
  ['items', 'duplicate_of', 'ALTER TABLE items ADD COLUMN duplicate_of INTEGER REFERENCES items(id)'],
  ['items', 'provider', 'ALTER TABLE items ADD COLUMN provider TEXT'],
]) {
...
const DEFAULTS = {
  // Fresh install: no digest until tomorrow morning — a 3pm "morning" digest
  // on day one reads as a bug.
  last_digest_date: (() => {
    const d = new Date();
...
function upsertItem(item) {
  ...
  if (existing) {
    db.prepare(
      ...
      item.submitted_at ?? null, item.provider ?? null, raw, now, existing.id
    );
  ...
  const info = db.prepare(
    `INSERT INTO items (source, source_id, course_id, kind, title, url, due_at,
                        all_day, points, submitted, submitted_at, provider, raw,
    ...
    item.submitted ?? null, item.submitted_at ?? null, item.provider ?? null,
    raw, now, now
  );
  return { result: 'inserted', id: Number(info.lastInsertRowid) };
}

function createManualItem({ title, due_at = null, course_id = null, notes = '', estimate_min = null, url = null }) {
  const now = Date.now();
  const info = db.prepare(
    `INSERT INTO items (source, source_id, course_id, kind, title, notes, due_at,
                        estimate_min, url, first_seen, last_seen)
     VALUES ('manual', NULL, ?, 'task', ?, ?, ?, ?, ?, ?, ?)`
  ).run(course_id, title, notes, due_at, estimate_min, url, now, now);
...
function hydrateItem(row) {
  const { raw, also_json, ...rest } = row; // raw stays server-side; it can be large
  let end_at = null;
  if (row.source === 'gcal' && raw) {
    try { end_at = JSON.parse(raw).end || null; } catch { /* no end */ }
  }
  // Cross-source duplicates linked to this row (see sync.linkDuplicates).
  let also = [];
  if (also_json) {
    try {
      also = JSON.parse(also_json).map((a) => ({ ...a, submitted: a.submitted === null ? null : !!a.submitted }));
    } catch { /* none */ }
  }
  const submitted = row.submitted === null ? null : !!row.submitted;
  return {
    ...rest,
    end_at,
    also,
    ...
    hidden: !!row.hidden,
    submitted,
    // "Did anyone see a submission?" — the flag the UI and alerts should use.
    submitted_any: submitted === true || also.some((a) => a.submitted === true),
  };
}
...
function listItems({
  ...
  where.push('i.duplicate_of IS NULL');
  ...
  return db.prepare(
    `SELECT i.id, i.source, i.source_id, i.course_id, i.kind, i.title, i.notes,
            ...
            i.provider, i.first_seen, i.missing_since,
            CASE WHEN i.source = 'gcal' THEN i.raw ELSE NULL END AS raw,
            (SELECT json_group_array(json_object('id', d.id, 'source', d.source, 'url', d.url, 'submitted', d.submitted))
               FROM items d WHERE d.duplicate_of = i.id) AS also_json,
...
function updateItem(id, fields) {
  ...
  if (row.source === 'manual') {
    ...
    if (fields.url !== undefined) { sets.push('url = ?'); params.push(fields.url ? String(fields.url) : null); }
  }
...
function listBlocks({ from, to }) {
  return db.prepare(
    `SELECT b.*, i.title AS item_title, i.status AS item_status,
            ...
            i.url AS item_url, c.code AS course_code
...
function stats() {
  ...
  const completedThisWeek = db.prepare(
    `SELECT COUNT(*) n FROM items WHERE status = 'done' AND completed_at >= ? AND duplicate_of IS NULL`
  ).get(weekStart).n;
  ...
  const perCourse = db.prepare(
    `SELECT c.id, c.code, c.name,
     ...
     JOIN items i ON i.course_id = c.id
       AND i.hidden = 0 AND i.duplicate_of IS NULL
