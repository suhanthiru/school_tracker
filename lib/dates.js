...
function findDates(text, now = Date.now()) {
  ...
  for (const m of s.matchAll(RE_WD)) {
    ...
    // A weekday naming today means next week — people say "today"/"tonight"
    // when they mean today ("deadline extended to Monday", posted Monday).
    if (days === 0) days = 7;
...
function parseCapture(text, now = Date.now()) {
  ...
  if (!raw) return { title: '', due_at: null, url: null };

  // Pull a pasted link out first — both to attach it to the task and so
  // digits inside the URL can't be misread as dates.
  const urlMatch = raw.match(/https?:\/\/\S+/);
  const url = urlMatch ? urlMatch[0].replace(/[),.;\]]+$/, '') : null;
  if (urlMatch) {
    raw = (raw.slice(0, urlMatch.index) + ' ' + raw.slice(urlMatch.index + urlMatch[0].length))
      .replace(/\s{2,}/g, ' ').trim();
    if (!raw) return { title: url, due_at: null, url };
  }

  const dates = findDates(raw, now);
  if (!dates.length) return { title: raw, due_at: null, url };
  ...
  return { title: title || raw, due_at: due, url };
}
